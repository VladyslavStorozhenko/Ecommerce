...
from ..user.hashing import verify_password
...
def authenticate_user(email: str, password: str, database: Session) -> models.User | bool:
...
def get_current_user(token: str = Depends(oauth2_scheme), database: Session = Depends(get_db)) -> schemas.DisplayUser:
